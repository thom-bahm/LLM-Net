...
class Agent:
    def __init__(self, task : str, identity: str):
        self.memory = []
...
def group_chat(npcs, goal, chat_type, max_rounds):
...
def spawn(number: int):
    ...
    agents = [Agent(identity) for identity in identities]
    return agents

task = '''You are a member of Bloomgberg's board of directors. You must find a solution for improving financial literacy in the city of New York. You must come to a unified conclusion.'''
agents = spawn(10)

print("Round-Robin Conversation:")
conversation_log = group_chat(agents, task, "round_robin", max_rounds=1)